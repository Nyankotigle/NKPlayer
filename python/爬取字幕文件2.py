...
StartMoviefolder = 'F:\\###电影F\\新建文件夹'

movieFolderList = []

for s in os.listdir(StartMoviefolder):                   #列出文件夹下的所有一级子文件夹完整路径
    movieFolderList.append(os.path.join(StartMoviefolder,s))
    #print(os.path.join(StartMoviefolder,s))
for movieFolder in movieFolderList:
    movieFileList = []
    sub_URL_List = []
    for m in os.listdir(movieFolder): 
        movieFileList.append(m)
        #print(m)
    print("文件夹", movieFolder)
    ...
    default_dir = movieFolder #+ "\\###字幕"
    if not os.path.exists(default_dir):
        os.mkdir(default_dir)            #当前字幕目录不存在则建立
